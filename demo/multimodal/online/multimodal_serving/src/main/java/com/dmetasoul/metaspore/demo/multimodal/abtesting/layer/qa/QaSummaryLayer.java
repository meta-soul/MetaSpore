...
@LayerAnnotation(name = "summary.qa")
@Component
public class QaSummaryLayer implements BaseLayer<SearchResult> {
    private LayerBucketizer bucketizer;

    @Override
    public void intitialize(LayerArgs layerArgs) {
        System.out.println("QA Summary layer, args:" + layerArgs);
    ...
    public String split(Context ctx, SearchResult in) {
        ...
        System.out.printf("QA Summary layer split: %s, return exp: %s%n", this.getClass().getName(), returnExp);
        return returnExp;
    }
}
