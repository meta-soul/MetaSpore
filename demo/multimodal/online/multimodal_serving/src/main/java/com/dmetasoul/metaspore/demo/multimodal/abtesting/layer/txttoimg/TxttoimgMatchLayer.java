...
@LayerAnnotation(name = "match.t2i")
@Component
public class TxttoimgMatchLayer implements BaseLayer<SearchResult> {
    private LayerBucketizer bucketizer;

    @Override
    public void intitialize(LayerArgs layerArgs) {
        System.out.println("[TxtToImg] Match layer, args:" + layerArgs);
    ...
    public String split(Context ctx, SearchResult in) {
        ...
        System.out.printf("[TxtToImg] Match layer split: %s, return exp: %s%n", this.getClass().getName(), returnExp);
        return returnExp;
    }
}
