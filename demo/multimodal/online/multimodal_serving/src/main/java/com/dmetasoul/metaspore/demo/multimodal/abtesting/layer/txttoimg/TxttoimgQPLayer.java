...
@LayerAnnotation(name = "qp.t2i")
@Component
public class TxttoimgQPLayer implements BaseLayer<SearchResult> {
    private LayerBucketizer bucketizer;

    @Override
    public void intitialize(LayerArgs layerArgs) {
        System.out.println("[TxtToImg] QP layer, args:" + layerArgs);
    ...
    public String split(Context ctx, SearchResult in) {
        ...
        System.out.printf("[TxtToImg] QP layer split: %s, return exp: %s%n", this.getClass().getName(), returnExp);
        return returnExp;
    }
}
