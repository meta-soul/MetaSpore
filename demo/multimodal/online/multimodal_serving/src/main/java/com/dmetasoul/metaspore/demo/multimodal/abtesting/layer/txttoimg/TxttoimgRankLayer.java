...
@LayerAnnotation(name = "rank.t2i")
@Component
public class TxttoimgRankLayer implements BaseLayer<SearchResult> {
    private LayerBucketizer bucketizer;

    @Override
    public void intitialize(LayerArgs layerArgs) {
        System.out.println("[TxtToImg] Rank layer, args:" + layerArgs);
    ...
    public String split(Context ctx, SearchResult in) {
        ...
        System.out.printf("[TxtToImg] Rank layer split: %s, return exp: %s%n", this.getClass().getName(), returnExp);
        return returnExp;
    }
}
